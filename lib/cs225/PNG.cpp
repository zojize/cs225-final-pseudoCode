/**
 * @file PNG.cpp
 * Implementation of a simple PNG class using RGBAPixels and the lodepng PNG
 * library.
 *
...
RGBAPixel& PNG::_getPixelHelper(unsigned int x, unsigned int y) const {
  if (width_ == 0 || height_ == 0) {
    cerr << "ERROR: Call to cs225::PNG::getPixel() made on an image with no "
            "pixels."
         << endl;
    assert(width_ > 0);
    assert(height_ > 0);
  ...
  if (x >= width_) {
    cerr << "WARNING: Call to cs225::PNG::getPixel(" << x << "," << y
         << ") tries to access x=" << x
...
RGBAPixel const& PNG::getPixel(unsigned int x, unsigned int y) const {
  return _getPixelHelper(x, y);
}
...
bool PNG::readFromFile(string const& fileName) {
  ...
  if (error) {
    cerr << "PNG decoder error " << error << ": " << lodepng_error_text(error)
         << endl;
    return false;
  }
...
bool PNG::writeToFile(string const& fileName) {
  ...
  if (error) {
    cerr << "PNG encoding error " << error << ": " << lodepng_error_text(error)
         << endl;
  }
...
std::ostream& operator<<(std::ostream& os, PNG const& png) {
  ...
  for (unsigned x = 0; x < png.width(); x++) {
    for (unsigned y = 0; y < png.height(); y++) {
      RGBAPixel const& pixel = png.getPixel(x, y);
